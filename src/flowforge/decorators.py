# src/flowforge/decorators.py  (or wherever your decorator lives)
...
def model(
    ...
    def deco(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        if src is None:
            try:
                src = inspect.getfile(func)  # type: ignore[assignment]
            except Exception as e:
                raise ModuleLoadError(
                    f"Cannot determine source path for model '{fname}': {e}"
                ) from e

        func.__ff_path__ = Path(cast(str, src)).resolve()

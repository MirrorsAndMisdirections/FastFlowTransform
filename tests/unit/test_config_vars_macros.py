...
class _DummyExec(BaseExecutor[Any]):
    def __init__(self) -> None:
        ...
        self.last_view: Optional[Tuple[str, str, str]] = None
        self.last_table: Optional[Tuple[str, str, str]] = None
        self.last_py_view: Optional[Tuple[str, str, str]] = None
    ...
    def _create_or_replace_view(self, target_sql: str, body: str, node: Node) -> None:
        ...
        setattr(self, "last_view", (target_sql, body, node.name))
    def _create_or_replace_table(self, target_sql: str, body: str, node: Node) -> None:
        setattr(self, "last_table", (target_sql, body, node.name))
    def _create_or_replace_view_from_table(self, view_name: str, backing_table: str, node: Node) -> None:
        setattr(self, "last_py_view", (view_name, backing_table, node.name))
    # unused by these tests:
    def _read_relation(self, relation: str, node: Node, deps): raise NotImplementedError
    def _materialize_relation(self, relation: str, df: Any, node: Node) -> None: raise NotImplementedError
    def _columns_of(self, frame: Any) -> list[str]: return []
    def _is_frame(self, obj: Any) -> bool: return False
...
def _setup_env(models_dir: Path) -> None:
    ...
    setattr(REGISTRY, "macros", getattr(REGISTRY, "macros", {}))
    setattr(REGISTRY, "cli_vars", {})
...
def test_config_hook_sets_materialized_view(tmp_path: Path):
    ...
    REGISTRY.nodes["users.ff"] = Node(name="users.ff", kind="sql", path=models/"users.ff.sql", deps=[], meta={})
...
def test_var_reads_project_and_cli_override(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    REGISTRY.nodes.clear()
    REGISTRY.nodes["echo_day.ff"] = Node("echo_day.ff", "sql", models/"echo_day.ff.sql", [], meta={})

    # make project vars visible to REGISTRY (your impl likely reads on load; we inject here)
    setattr(REGISTRY, "project_vars", {"day": "2000-01-01"})
    # CLI override
    setattr(REGISTRY, "cli_vars", {"day": "2025-10-01"})
    ...
    target, body, _ = ex.last_table  # created as table by default
...
def test_macros_are_loaded_and_callable(tmp_path: Path):
    ...
    setattr(REGISTRY, "macros", {"my_macro": macros / "my_macro.sql"})
